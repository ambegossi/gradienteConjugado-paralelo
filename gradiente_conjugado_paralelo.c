...
void multiplicacaoMatrizVetor(int M, int N, int *colptr, int *linhas, double *a, 
		double *b, double *q, int num_threads, double **qm)
{
...
#pragma omp parallel private(i, j, id)
    {
        ...
        for (i = id; i < N; i += num_threads)
        {
            for (j = colptr[i] - 1; j < colptr[i + 1] - 1; j++)
            {
                if (j != colptr[i] - 1)
                {
                    qm[linhas[j] - 1][id] += a[j] * b[i];
                }

                qm[i][id] += a[j] * b[linhas[j] - 1];
            }
        }
    }
...
void gradiente_conjugado(int M, int N, int naozeros, int *colptr, int *linhas, 
		double *valores, double *b, int num_threads, double **qm)
{
    ...
    // aux = A * x
    multiplicacaoMatrizVetor(M, N, colptr, linhas, valores, x, aux, num_threads,qm);
    ...
    while (i < imax && sigma_novo > (erro * erro * sigma0))
    {
        ...
        // q = A * d
        multiplicacaoMatrizVetor(M, N, colptr, linhas, valores, d, q, num_threads,qm);
    ...
    printf("\nGradiente finalizado.\n");
    printf("nº de iteracoes = %d\n", i);
...
int main(int argc, char **argv)
{
    ...
    double **qm = (double **)malloc(M * sizeof(double *)); 
    for (i = 0; i < M; i++)
    {
        qm[i] = (double *)malloc(num_threads * sizeof(double)); 
    }

    printf("\nO gradiente está sendo calculado...\n");
    double ti = omp_get_wtime();

    gradiente_conjugado(M, N, naozeros, colptr, linhas, valores, b, num_threads, qm);
